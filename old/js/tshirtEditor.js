...
$(document).ready(function () {
	...
	window.onload = function () {
		...
		var productId = url.split("/index.html?")[1];
		console.log(productId);

		// Call the API
		fetch("http://localhost:4000/canvasimage/" + productId)
			.then(response => response.json())
			.then(data => {
				if (data && data.arr && data.arr.length > 0) {
					console.log(data);
					FimageUrl = data.arr[0].frontimageurl;
					BimageUrl = data.arr[0].backimageurl;
